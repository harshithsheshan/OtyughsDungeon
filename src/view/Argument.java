...
public class Argument extends JFrame {
  ...
  public Argument(GuiControllerInterface listener) {
    ...
    add(message);
    row = new JSpinner(new SpinnerNumberModel(7, 5, 100, 1));

    //row = new JTextField();
    add(row);
    message = new JLabel("Cols:");
    add(message);
    col = new JSpinner(new SpinnerNumberModel(7, 5, 100, 1));
    add(col);
    //6 8 4 true 40 3 0
    message = new JLabel("Interconnectivity:");
    add(message);
    inter = new JSpinner(new SpinnerNumberModel(5, 4, 100, 1));
    add(inter);
    ...
    add(message);
    otyugh = new JSpinner(new SpinnerNumberModel(2, 1, 10, 1));
    add(otyugh);
